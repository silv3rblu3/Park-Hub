...
const CoreSystem = {
    ...
    stopActiveScanner: async function() {
        if (this.activeScanner) {
            try { 
                await this.activeScanner.stop(); 
            } catch (e) { 
                /* Ignore errors if already stopping */ 
            }
            try { 
                this.activeScanner.clear(); 
            } catch (e) { 
                /* Ignore clear errors */ 
            }
    ...
    bindGlobalEvents: function() {
        ...
        document.addEventListener('click', (e) => {
            if (!bentoMenu.classList.contains('hidden') && !bentoTrigger.contains(e.target) && !bentoMenu.contains(e.target)) {
        ...
        // --- 3. Global Settings & Theme Engine ---
        ...
        document.getElementById('global-theme-select').addEventListener('change', (e) => {
            this.loadThemeIntoEditor(e.target.value);
        });
        ...
        document.getElementById('new-theme-btn').addEventListener('click', () => {
            ...
            ['bg', 'surface', 'text-main', 'text-muted', 'accent', 'accent-hover', 'border', 'danger'].forEach(id => {
                document.getElementById(`theme-edit-${id}`).value = '#000000';
            });
        });
        ...
        document.getElementById('save-theme-btn').addEventListener('click', () => {
            ...
            if(!name) {
                return NotificationSystem.show("Theme requires a name", "error");
            }
        ...
        document.getElementById('delete-theme-btn').addEventListener('click', () => {
            ...
            if (state.themes.length <= 1) {
                return NotificationSystem.show("Cannot delete last theme", "error");
            }
            if (state.activeThemeId === id) {
                return NotificationSystem.show("Cannot delete active theme. Switch first.", "error");
            }
        ...
        document.getElementById('btn-open-export-modal').addEventListener('click', () => {
            if (typeof QRCode === 'undefined') { 
                alert("ERROR: QRCode generation library missing. Check js/qrcode.min.js"); 
                return; 
            }
            ...
            if (chunks.length === 0) {
                return NotificationSystem.show("No data to export", "error");
            }
        ...
        document.getElementById('close-qr-export-btn').addEventListener('click', () => {
            ...
            if (this.qrExportTimer) {
                clearTimeout(this.qrExportTimer);
            }
            exportModal.close();
            settingsModal.showModal();
        });

        // --- 5. Data Sync: Import QR ---
        ...
        document.getElementById('btn-open-import-modal').addEventListener('click', () => {
            if (typeof Html5Qrcode === 'undefined') { 
                alert("ERROR: Html5Qrcode scanner library missing."); 
                return; 
            }
                            ...
                            if (scannedIndices.size === totalExpected) {
                                this.stopActiveScanner().then(() => {
                                    ...
                                    DialogSystem.confirm(`Syncing ${targetApp}`, `All data chunks captured successfully. Click OK to safely MERGE this data into your device.`).then(proceed => {
                                        if (proceed) {
                                            StateManager.processStringImport(assembledString, targetApp, 'merge');
                                        } else {
                                            settingsModal.showModal();
                                        }
                                    });
        ...
        document.getElementById('btn-generate-share').addEventListener('click', () => {
            if (typeof QRCode === 'undefined') { 
                alert("ERROR: QRCode library missing."); 
                return; 
            }
        ...
        // --- 7. Data Sync: File Exports & Imports ---
        ...
        const handleSmartImport = async (e, mode) => {
            ...
            if (fileNameParts.length >= 2 && fileNameParts[1] === 'PMH') {
                ...
                // Target Map explicitly includes roster routing to prevent global overrides
                const targetMap = { 
                    'global': 'global', 
                    'themes': 'themes', 
                    'parkinfo': 'parkInfo', 
    ...
    executeGlobalScanSearch: function(scannedId) {
        ...
        if (!found) {
            DialogSystem.alert("Not Found", `No item found matching ID: ${scannedId}`);
        }
    },
    ...
    generateHomeDashboard: function() {
        ...
        // --- Added Roster Analytics ---
        let checkedIn = 0;
        let pendingArr = 0;
        let troubleSites = 0;
        if (state.apps.roster && state.apps.roster.active) {
            state.apps.roster.active.forEach(c => {
                if (c.status === 'Checked-In') checkedIn++;
                if (c.status === 'Pending') pendingArr++;
                if (c.isTrouble) troubleSites++;
            });
        }
            ...
            state.apps.inventory.items.forEach(item => {
                ...
                (state.apps.inventory.transactions || []).filter(t => t.sku === item.sku).forEach(t => {
                    if(t.type === 'Stock In') {
                        ...
                        qty -= Number(t.quantity);
                    } else if(t.type === 'Audit Correction') {
                        qty = Number(t.quantity);
                    }
                });
        ...
        if (state.apps.parts && state.apps.parts.partsCatalog) {
            state.apps.parts.partsCatalog.forEach(p => {
                ...
                if (p.isCritical && Number(p.qty) === 0) {
                    criticalOut++;
                }
            });
        }
        ...
        return `
            <div style="padding: 2rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
                    ...
                    <div class="app-card searchable-card" style="cursor: pointer; border-top: 4px solid var(--accent-primary); transition: transform 0.2s;" onclick="CoreSystem.routeToApp('roster')" onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'">
                        <h3 style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;"><span style="font-size: 1.5rem;">⛺</span> Camp Roster</h3>
                        <div style="display: flex; flex-direction: column; gap: 5px;">
                            <p><strong>${checkedIn}</strong> Checked-In</p>
                            <p><strong>${pendingArr}</strong> Pending Arrival</p>
                            ${troubleSites > 0 ? `<p><strong style="color: var(--danger-color); font-size: 1.1rem;">⚠️ ${troubleSites} Trouble Site(s)</strong></p>` : `<p style="color: var(--text-secondary);">No issues reported.</p>`}
                        </div>
                    </div>
    ...
    routeToApp: function(appName) {
        ...
        switch(appName) {
            case 'home': 
                ...
                break;
            case 'roster': 
                titleLabel.innerText = "Camp Roster"; 
                if (typeof renderRosterApp === 'function') { 
                    container.innerHTML = renderRosterApp(); 
                    if (typeof initRosterLogic === 'function') {
                        initRosterLogic(); 
                    }
                } 
                break;
            case 'parkInfo': 
                ...
                if (typeof renderParkInfoApp === 'function') { 
                    container.innerHTML = renderParkInfoApp(); 
                    if (typeof initParkInfoLogic === 'function') {
                        initParkInfoLogic(); 
                    }
                } 
                break;
            case 'inventory': 
                ...
                if (typeof renderInventoryApp === 'function') { 
                    container.innerHTML = renderInventoryApp(); 
                    if (typeof initInventoryLogic === 'function') {
                        initInventoryLogic(); 
                    }
                } 
                break;
            case 'fleet': 
                ...
                if (typeof renderFleetApp === 'function') { 
                    container.innerHTML = renderFleetApp(); 
                    if (typeof initFleetLogic === 'function') {
                        initFleetLogic(); 
                    }
                } 
                break;
            case 'winterization': 
                ...
                if (typeof renderWinterizationApp === 'function') { 
                    container.innerHTML = renderWinterizationApp(); 
                    if (typeof initWinterizationLogic === 'function') {
                        initWinterizationLogic(); 
                    }
                } 
                break;
            case 'firstAid': 
                titleLabel.innerText = "First Aid Tracker"; 
                if (typeof renderFirstAidApp === 'function') { 
                    container.innerHTML = renderFirstAidApp(); 
                    if (typeof initFirstAidLogic === 'function') {
                        initFirstAidLogic(); 
                    }
                } 
                break;
            case 'parts': 
                ...
                if (typeof renderPartsApp === 'function') { 
                    container.innerHTML = renderPartsApp(); 
                    if (typeof initPartsLogic === 'function') {
                        initPartsLogic(); 
                    }
                } 
                break;
            case 'projects': 
                ...
                if (typeof renderProjectsApp === 'function') { 
                    container.innerHTML = renderProjectsApp(); 
                    if (typeof initProjectsLogic === 'function') {
                        initProjectsLogic(); 
                    }
                } 
                break;
